...
getShmReadAndWriteOps(Operation *parentOp, Value shmMemRef,
  ...
  if (llvm::any_of(writeOps, [](Operation *op) {
        return !isa<memref::StoreOp, vector::StoreOp, vector::TransferWriteOp> (op) ||
               getIndices(op).size() < 2;
      }))
    return failure();
  return success();
...
LogicalResult optimizeSharedMemoryReadsAndWrites(Operation *parentOp,
  ...
  if (!memRefType || !amdgpu::AMDGPUDialect::hasSharedMemoryAddressSpace(memRefType)
  )
    return failure();
...
void testSharedMemoryReadsAndWrites(Operation *op) {
    ...
    for (auto allocOp : shmAllocOps) {
      if (failed(optimizeSharedMemoryReadsAndWrites(op,
                                                    allocOp.getMemref())))
        return;
...
namespace {
struct OptimizeSharedMemoryReadsAndWritesPass
    : public OptimizeSharedMemoryReadsAndWritesPassBase<OptimizeSharedMemoryReadsAndWritesPass> {
...
createOptimizeSharedMemoryReadsAndWritesPass() {
  return std::make_unique<OptimizeSharedMemoryReadsAndWritesPass>();
}
